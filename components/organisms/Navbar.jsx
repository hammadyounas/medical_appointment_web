  ...
  return (
    <nav className="lg:p-10 sm:p-5 p-2 flex justify-between items-center">
      <div className="flex max-md:justify-between items-center w-full">
        ...
        >
          <Dropdown options={languageOptions} label="Language"   onSelect={(option) => handleSelect(option)} className={'whitespace-nowrap'}/>
          {/* <Button bg={"bg-primary"} text={"Appointment"} color={"text-white"} rounded="lg" /> */}
          <Icon icon="mdi:menu" className="text-3xl cursor-pointer z-50" onClick={() => setIsOpen(!isOpen)}/>
        </div>
      ...
      <div className="z-10 max-md:hidden flex items-center gap-2">
        <Dropdown options={languageOptions} label="Language"  onSelect={(option) => handleSelect(option)} />
        <Button bg={"bg-white"} text={t.navbar.appointment} color={"text-primary"} rounded="lg" className='whitespace-nowrap' />
      </div>
    </nav>
  );
}
