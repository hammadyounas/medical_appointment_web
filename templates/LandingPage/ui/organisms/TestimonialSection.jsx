...
export default function TestimonialSection({selectedIndex, setSelectedIndex, t}) {
  ...
  return (
    <section className="bg-gray-100 py-3 px-6 lg:px-32">
      <h2 className="text-center lg:text-3xl sm:text-2xl text-xl font-bold text-gray-900">{t.reviews_title.heading}</h2>
      <p className="text-center text-primary mb-12 sm:text-base text-sm">{t.reviews_title.subHeading}</p>
      <div className="flex flex-col md:flex-row gap-8 items-center">
        {/* Left Section: Clients List */}
        <div className="w-full md:w-1/3 flex flex-col gap-6">
          {t.reviews.map((client, index) => (
            <div
              key={index}
        ...
        <div className="w-full md:w-2/3 text-gray-700">
          ...
          <p className="sm:text-lg italic">
            {t.reviews[selectedIndex].review}
          </p>
          <div className="mt-4 flex text-primary">
            {Array(t.reviews[selectedIndex].rating).fill("★").map((star, i) => (
